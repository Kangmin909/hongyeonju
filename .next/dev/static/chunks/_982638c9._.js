...
(()=>{
    const e = new Error("Cannot find module '../assets/icons/back-arrow.png'");
...
var Menu = function() {
    ...
    var nextPathRef = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].useRef(null);
    // 메뉴 닫기 함수
        ...
        "Menu.useEffect": function() {
            var handlePopState = {
                "Menu.useEffect.handlePopState": function(e) {
                    ...
                    // 1. 대기 중인 네비게이션이 있는 경우 (메뉴에서 다른 페이지 클릭 후 history.back이 완료된 시점)
                    if (nextPathRef.current) {
                        var targetPath = nextPathRef.current;
                        nextPathRef.current = null;
                        // 메뉴 상태를 끄고 새 페이지로 이동 (이 시점의 히스토리는 이미 깨끗함)
                        if (isMenuOpen) toggleMenu();
    ...
    var handleNavigate = function(path) {
        if (location.pathname === path) {
            // 동일 페이지: 단순히 히스토리를 뒤로 돌려 메뉴만 닫음
            window.history.back();
        } else {
            var _window_history_state;
            isNavigatingRef.current = true;
            nextPathRef.current = path; // 이동할 경로 예약
            if (((_window_history_state = window.history.state) === null || _window_history_state === void 0 ? void 0 : _window_history_state.modal) === 'menu') {
                window.history.back(); // 먼저 히스토리를 한 칸 뒤로(깨끗한 상태로) 보냄
            } else {
                // 예외 상황: 히스토리 상태가 꼬였을 경우 즉시 이동
                if (isMenuOpen) toggleMenu();
                router.push(path);
            }
        }
...
_s(Menu, "SCDr46o08mpda7KrPUvFmHcPCxQ=", false, function() {
    return [
...
//# sourceMappingURL=_982638c9._.js.map